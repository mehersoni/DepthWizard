...
def rank_shadow_regions(
    ...
    for r in regions:
        ...
        shape_score = compute_shadow_shape_score(
            elongation=r.get("elongation", 0.5),
            aspect_ratio=r.get("aspect_ratio", 1.0),
            solidity=r.get("solidity", 0.8),
            orientation_score=orient_score
        )

        area_reliability = compute_area_reliability(r.get("area", 100.0), a_ref=a_ref)

        complexity_score, merge_suspicion, r_struct = compute_complexity_and_merge_suspicion(
            solidity=r.get("solidity", 0.8),
            extent=r.get("extent", 0.5),
            aspect_ratio=r.get("aspect_ratio", 1.0),
            elongation=r.get("elongation", 0.5),
            area=r.get("area", 100.0)
        )
